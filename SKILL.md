...
description: 生成以内容为主的 Markmap 思维导图，用于项目、文档、代码库或概念梳理。Use when Codex needs to create Markdown/HTML mind maps that explain source material from whole to parts, keep real content as the structure, and by default limit every level to at most 5 direct children unless the user explicitly allows more.
---
...
- 每个节点的直接子节点默认最多 5 个，这是默认硬约束，不是建议值。
- 只有用户明确声明“可以更多分支/可以超过 5 个/最多 N 个”时，才允许提高上限；提高后仍应尽量克制。
- 不要因为真实内容很多就平铺超过 5 个分支；应按材料里的真实子主题继续分组。
- 如果某个层级看起来需要超过 5 个分支，先合并相近内容，再按“整体 -> 分组 -> 局部细节”重排。
...
2. 从整体到局部起草 Markdown。
   ...
   - 每个父节点的直接子节点默认最多 5 个；超过 5 个时按真实子主题重新分组。
3. 用脚本校验 Markdown：
...
如果用户明确允许更多分支，才可指定更高上限：

```bash
python3 ~/.codex/skills/markmap-analogy-mindmap/scripts/validate_and_render.py input.md --max-children 7 --validate-only
```
...
5. 如果校验失败，先修导图结构。除非用户明确要求非标准导图，否则不要绕过“空泛分类”和“层级分支过多”检查。
